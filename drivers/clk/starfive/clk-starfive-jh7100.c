...
static const struct clk_div_table sdio1_cclkint_div_table[] = {
    { .val = 6, .div = 6, },
    { }
};
...
static int __init starfive_clkgen_init(struct clk_starfive_jh7100_priv *priv)
{
	...
	hws[JH7100_CLK_ISP0_BUS]		= starfive_clk_divider(priv, "isp0_bus",	"vin_src",	0x18c, 4);
	...
	hws[JH7100_CLK_ISP1_BUS]		= starfive_clk_divider(priv, "isp1_bus",	"vin_src",	0x19c, 4);
	...
	hws[JH7100_CLK_VIN_BUS]			= starfive_clk_divider(priv, "vin_bus",	"vin_src",	0x1a8, 4);
...
*/
	...
	hws[JH7100_CLK_SDIO1_CCLKINT]       = clk_hw_register_divider_table(priv->dev,  "sdio1_cclkint",	"perh1_src",
		                         CLK_SET_RATE_PARENT | CLK_SET_RATE_GATE, priv->base + 0x1d8, 0, 5, 0, sdio1_cclkint_div_table, &priv->rmw_lock);
	clk_prepare_enable(hws[JH7100_CLK_SDIO1_CCLKINT]->clk);
	...
	hws[JH7100_CLK_QSPI_REF]		= starfive_clk_gated_divider(priv, "qspi_refclk",	"perh0_src",	0x22c, 5);
